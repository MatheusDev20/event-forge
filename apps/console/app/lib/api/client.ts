 ...
 * client, react-query owns caching.
 */
const IS_SERVER = typeof window === 'undefined';

if (!BASE_URL) {
  throw new Error(
    'NEXT_PUBLIC_BACKEND_URL is not set — copy apps/console/.env.example to .env.local',
  );
}
 ...
 *
 * A sibling of the storefront's client rather than a shared package, and the
 * duplication is the point: the two apps disagree about caching. `apps/web`
 * caches catalog reads for a minute because a listing that is slightly stale
 * costs a visitor nothing. This one never caches — an organizer who publishes
 * an event and sees it still marked "draft" has been told a lie by their own
 * console, and no revalidation window makes that acceptable.
 *
 * Extracting the shared half would mean parameterising exactly the behaviour
 * that must not be shared. If a third consumer appears, revisit.
 *
...
export async function apiGet<T>(
  ...
  try {
    response = await fetch(url, {
      ...
      // `no-store`, not a revalidate window: see the note above.
      ...(IS_SERVER ? { cache: 'no-store' as const } : {}),
    });
  } catch (cause) {
